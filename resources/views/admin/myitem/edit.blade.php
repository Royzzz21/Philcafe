...
@section('content')
    ...
    {!! Form::model($company, ['method' => 'PUT', 'route' => ['admin.myitem.update', $company->id], 'files' => true,]) !!}
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            ...
            <div class="row">
                <div class="col-xs-12 form-group">
                    {!! Form::label('categories', trans('quickadmin.items.fields.categories').'', ['class' => 'control-label']) !!}
                    {!! Form::select('categories[]', $categories, old('categories') ? old('categories') : $company->subcategories->pluck('id')->toArray(), ['class' => 'form-control select2', 'multiple' => 'multiple']) !!}
                    <p class="help-block"></p>
            ...
            <div class="row">
                <div class="col-xs-12 form-group">
                    @if ($company->logo)
                        <a href="{{ asset(env('UPLOAD_PATH').'/'.$company->logo) }}" target="_blank"><img src="{{ asset(env('UPLOAD_PATH').'/' . $company->logo) }}" width="100" height="50"></a>
                    @endif
            ...
            <div class="row">
                <div class="col-xs-12 form-group">
                    @if ($company->logo)
                        <a href="{{ asset(env('UPLOAD_PATH').'/'.$company->logo) }}" target="_blank"><img src="{{ asset(env('UPLOAD_PATH').'/' . $company->logo1) }}" width="100" height="50"></a>
                    @endif
                    {!! Form::label('logo', trans('quickadmin.items.fields.logo1').'', ['class' => 'control-label']) !!}
                    {!! Form::hidden('logo', old('logo')) !!}
                    {!! Form::file('logo', ['class' => 'form-control', 'style' => 'margin-top: 4px;']) !!}
                    {!! Form::hidden('logo_max_size', 5) !!}
                    {!! Form::hidden('logo_max_width', 4096) !!}
